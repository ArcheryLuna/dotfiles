...
module.exports = function (argv) {
    ...
    program
        ...
        .action(({ yarn, packagedDependencies, ignoreFile, dependencies }) => main(package_1.ls({ useYarn: yarn, packagedDependencies, ignoreFile, dependencies })));
    program
        ...
        .option('--no-dependencies', 'Disable dependency detection via npm or yarn')
        .action((version, { out, target, message, gitTagVersion, updatePackageJson, githubBranch, gitlabBranch, baseContentUrl, baseImagesUrl, yarn, ignoreFile, gitHubIssueLinking, gitLabIssueLinking, dependencies, }) => main(package_1.packageCommand({
        packagePath: out,
        version,
        target,
    ...
    program
        ...
        .option('--ignoreFile <path>', 'Indicate alternative .vscodeignore')
        .option('--no-dependencies', 'Disable dependency detection via npm or yarn')
        .action((version, { pat, target, message, gitTagVersion, updatePackageJson, packagePath, githubBranch, gitlabBranch, baseContentUrl, baseImagesUrl, yarn, noVerify, ignoreFile, dependencies, }) => main(publish_1.publish({
        pat,
        version,
        targets: target,
        commitMessage: message,
