...
class PathManager {
    ...
    rebuildWorkspaceNamePathMap(rootPath) {
        let beginMS = tools_1.Tools.getCurrentMS(); //启动时毫秒数
        ...
        for (let processingFileIdx = 0; processingFileIdx < workspaceFileCount; processingFileIdx++) {
            ...
            if (_fileNameToPathMap[fileNameKey]) {
                //存在同名文件
                if ((0, util_1.isArray)(_fileNameToPathMap[fileNameKey])) {
        ...
        let endMS = tools_1.Tools.getCurrentMS(); //文件分析结束时毫秒数
    ...
    checkSameNameFile(distinguishSameNameFile) {
        ...
        for (const nameKey in this.fileNameToPathMap) {
            ...
            if ((0, util_1.isArray)(completePath)) {
                //初始化语句
...
PathManager.rootFolderArray = {}; // name uri
