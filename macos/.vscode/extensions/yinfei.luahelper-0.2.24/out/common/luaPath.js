...
class LuaPath {
    ...
    // 获取插件自带的lua二进制完整路径
    getLuaExePathStr(vSCodeExtensionPath, luaVersionStr) {
        let path = require("path");
        ...
        switch (platform) {
            case "win32":
                let suffixVerStr = luaVersionStr.replace(".", "");
    ...
    getLuaCPathStr(vSCodeExtensionPath, luaVersionStr) {
        let path = require("path");
        let pathArr = vSCodeExtensionPath.split(path.sep);
        let stdPath = pathArr.join('/');
        let retStr = "";
    ...
    GetLuaExeCpathStr(strPath, vSCodeExtensionPath) {
        let strVect = ["", ""];
        let strVer = "";
        if (strPath === "") {
            ...
            if (luaVersionStr === "") {
                ...
                let newVSCodeExtensionPath = this.getSetLuaPath(vSCodeExtensionPath);
            ...
            // 用户设置了lua二进制的位置
            let strNewPath = this.getSetLuaPath(strPath);
...
exports.LuaPath = LuaPath;
//# sourceMappingURL=luaPath.js.map