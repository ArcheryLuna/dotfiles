...
function activate(context) {
    ...
    tools_1.Tools.SetVSCodeExtensionPath(context.extensionPath);
    // left progess bar
...
function doStartServer() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        var requirePathSeparator = ".";
        if (lspConfig !== undefined) {
            requirePathSeparator = requirePathSeparatorConfig;
        }
        ...
        if (enableReportConfig !== undefined) {
            enableReportFlag = enableReportConfig;
        }
        // 定义所有的监控文件后缀的关联
        ...
        let ignoreFileOrDirArr = vscode.workspace.getConfiguration("luahelper.base", null).get("ignoreFileOrDir");
        let ignoreFileOrDirErrArr = vscode.workspace.getConfiguration("luahelper.base", null).get("ignoreFileOrDirError");
        const clientOptions = {
...
function copyLuaSocket() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let strTitle = "Please select the lua version to copy luasocket";
        if (vscode.env.language === "zh-cn" || vscode.env.language === "zh-tw") {
            strTitle = "请选择要拷贝luasocket的lua版本";
        }
        let selectWord = yield vscode.window.showQuickPick(arr, {
        ...
        try {
            if (process.platform === "win32") {
                srcCopyDir = path.join(tools_1.Tools.VSCodeExtensionPath, '/debugger/luasocket/win/x64/' + selectWord.label + "/socket");
                let cmdStr1 = "xcopy " + srcCopyDir + " " + dstPath + "\\socket\\" + " /S /Y";
                console.log("cmdStr:%s", cmdStr1);
                child_process.execSync(cmdStr1);
                srcCopyDir = path.join(tools_1.Tools.VSCodeExtensionPath, '/debugger/luasocket/win/x64/' + selectWord.label + "/mime");
                ...
                srcCopyDir = path.join(tools_1.Tools.VSCodeExtensionPath, '/debugger/luasocket/mac/' + selectWord.label + "/socket");
                let cmdStr1 = "cp -R " + srcCopyDir + " " + dstPath + "/";
                console.log("cmdStr:%s", cmdStr1);
                child_process.execSync(cmdStr1);
                srcCopyDir = path.join(tools_1.Tools.VSCodeExtensionPath, '/debugger/luasocket/mac/' + selectWord.label + "/mime");
                ...
                srcCopyDir = path.join(tools_1.Tools.VSCodeExtensionPath, '/debugger/luasocket/linux/' + selectWord.label + "/socket");
                let cmdStr1 = "cp -a " + srcCopyDir + " " + dstPath + "/";
                console.log("cmdStr:%s", cmdStr1);
                child_process.execSync(cmdStr1);
                srcCopyDir = path.join(tools_1.Tools.VSCodeExtensionPath, '/debugger/luasocket/linux/' + selectWord.label + "/mime");
