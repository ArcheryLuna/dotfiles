...
class luacheck {
    ...
    invokeCheckFunction(fn, source, max_reports, convert_column) {
        function createReport(luadata) {
            let report = {
                code: "",
                column: 0,
                end_column: 0,
    ...
    check(filepath, source_text = null, max_reports = 100, utf8position = false) {
        let lines = null;
        if (source_text) {
        ...
        function convertColumn(line, column) {
            let t = lines[line - 1];
            let utf8len = 0;
            let codeCount = 0;
